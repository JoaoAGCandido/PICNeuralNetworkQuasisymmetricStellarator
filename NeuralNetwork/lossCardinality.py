...
parser = argparse.ArgumentParser(
    description="Create neural network with different number of points,\nexample:\npython3 NeuralNetwork/lossCardinality.py -v -nfp=3 -ds=\"scans/scan7/scan7.csv.zip\" -f=\"NeuralNetwork/neuralNetwork\"")
parser.add_argument(
...
for i in np.arange(0.1, 1.05, 0.05).round(2):
    if i != 1.00:
        # reduce size of training set
        X_train_scaled_reduced, X_unused, y_train_scaled_reduced, y_unused = \
        train_test_split(X_train_scaled, y_train_scaled, train_size=i,
                        random_state=0)
    else:
        X_train_scaled_reduced=X_train_scaled
        y_train_scaled_reduced=y_train_scaled
    ...
    # Train
    neuralNetwork.fit(X_train_scaled_reduced, y_train_scaled_reduced)

    #test
    Y_NN = neuralNetwork.predict(X_test_scaled)
    out["percentage"].append(i)
    out["numberPoints"].append(len(X_train_scaled_reduced))
    out["testR2"].append(r2_score(y_test_scaled, Y_NN))
    out["testMse"].append(mean_squared_error(y_test_scaled, Y_NN))
df = pd.DataFrame(out)
